...
If your MDA file policies apply to third-party cloud apps connected via API connectors, migration to Purview is supported for **Box**, **Dropbox**, **Salesforce**, and **Google Workspace**. You need to onboard those connectors in the Microsoft Defender portal and then create policies in Microsoft Purview.
...
> [!NOTE]
> This step is required only for apps that are not already connected to Defender for Cloud Apps. For third-party connected apps, no additional Microsoft Purview integration steps are required beyond setting up the connector. Once connected, all files from the third-party app are automatically in scope for Purview DLP and auto-labeling policies.

1. Go to [Microsoft Defender portal](https://security.microsoft.com) > **System** > **Settings** > **Cloud Apps** > **Connected apps** > **App connectors**.
1. Select **Connect an app** and choose the app from the list (supported apps: **Box**, **Dropbox**, **Salesforce**, and **Google Workspace**).
1. Enter the required instance name and credentials to complete the connection.
1. Go to **System** > **Settings** > **Cloud Apps** > **Information Protection** > **Files** and ensure **file monitoring is enabled**.
...
1. On **Locations**, select **Instances**. Edit to scope to specific instances or all instances.
1. Select the workload location and the specific connected app instance to scope the policy.
1. Create a rule with a name and conditions (such as sensitive information types).
1. Select **Add an action** and select **Restrict third-party apps**.
   - Each connected app has supported governance actions that can be taken. Select the actions appropriate for the app.
1. Review and save the policy. Run in **simulation mode** as a best practice.

### Step 3: Create an auto-labeling policy for connected sources

> [!IMPORTANT]
> Auto-labeling policies for third-party connected sources are **not yet available** in Microsoft Purview. The steps below apply to **first-party workloads** (SharePoint Online and OneDrive for Business) only. For third-party connected apps, this step is not currently supported. Check back for updates as this capability is being developed.
